...
# Methodology
1. Data Preparation
The dataset was imported using Pandas and divided into input features (ACL, TAR, CPI, Paq, and OEP) and target variables (Plant Type and Climate).

2. Train-Test Split
The dataset was split into training and testing subsets using an 80:20 ratio to evaluate model performance on unseen data.

3. Model Development
A Random Forest Classifier was implemented using Scikit-learn with 100 decision trees (n_estimators = 100).

4. Prediction
The trained model was used to predict vegetation type and climate conditions for new samples based on their alkane proxy values.
...
# Future Improvements
Train the model using real-world GC-MS datasets.
Expand the number of biomarker proxies and environmental variables.
Compare Random Forest performance with other machine learning algorithms such as XGBoost and Neural Networks.
Develop visualization tools for paleoenvironmental interpretation.
Integrate additional geological and climatic datasets for improved predictive capability.
